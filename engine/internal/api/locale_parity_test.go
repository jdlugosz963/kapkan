...
func TestLocaleParityParserSelfCheck(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"strings", 0, 472}, // +23: nav.nodes, col.node, nd.*; +32: nav.edge, ed.* (E4.5); +12: ed.h3* (E5.5); +34: ed.tenant/placement + nav.edgenodes + en.* (E6.7 fleet); +36: ed.hist/srcs/ev/state (E6.7 history); +34: ed.lever.* (E6.8); +2: ho.top10/notraffic; +1: btn.docs; +2: se.docs; +3: ov upstream metric; +20: Open Peering; +2: attack OpenPeering MAC remainder labels
		{"enums", 1, 9},
